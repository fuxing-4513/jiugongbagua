...
export default function HuangliClient() {
  ...
  return (<>
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold text-gold-600 font-serif mb-3">{getT('modules.huangli.name')}</h1>
      <p className="text-gray-600 mb-8">{getT('modules.huangli.desc')}</p>

      {/* Date Navigator */}
      <div className="bg-white rounded-xl border border-gold-500/25 p-4 mb-6">
        <div className="flex items-center justify-between">
          <button onClick={prevDay} className="w-10 h-10 flex items-center justify-center rounded-lg bg-gold-500/5 text-gold-700 hover:bg-gold-500/10 transition-colors">
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-1">
              <input
                ...
                className="w-20 px-2 py-1 text-center border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-gold-500/25"
                min={1900} max={2100}
              />
              ...
              <input
                ...
                className="w-16 px-2 py-1 text-center border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-gold-500/25"
                min={1} max={12}
              />
            ...
            <button onClick={goToday} className="text-xs bg-gold-500/10 text-gold-700 px-2 py-1 rounded hover:bg-gold-500/10 transition-colors">
              {getT('huangliPage.goToday')}
            </button>
          </div>

          <button onClick={nextDay} className="w-10 h-10 flex items-center justify-center rounded-lg bg-gold-500/5 text-gold-700 hover:bg-gold-500/10 transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Main Huangli Card */}
      <div className="bg-white rounded-xl border border-gold-500/25 p-4 mb-4">
        <h2 className="text-lg font-semibold text-gold-600 mb-2">{getT('modules.huangli.today')}</h2>
        <p className="text-sm text-gray-500 mb-4">{data.dateStr} {data.weekDay}</p>

        {/* Quick Info */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-gold-500/5 rounded-lg p-3 text-center">
        ...
        {/* Gan-Zhi */}
        <div className="bg-gradient-to-r from-gold-500/5 to-gold-500/5 rounded-xl p-4 mb-6 border border-gold-500/25">
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              <p className="text-xs text-gray-500 mb-1">{getT('huangliPage.yearPillarLabel')}</p>
              <p className="text-lg font-bold text-gold-600 font-serif">{data.ganZhiYear}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">{getT('huangliPage.monthPillarLabel')}</p>
              <p className="text-lg font-bold text-gold-600 font-serif">{data.ganZhiMonth}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">{getT('huangliPage.dayPillarLabel')}</p>
              <p className="text-lg font-bold text-gold-600 font-serif">{data.ganZhiDay}</p>
            </div>
          </div>
        </div>
          ...
          <div className="mb-4">
            <div className="flex flex-wrap gap-2">
              {data.lunarFestival.map((f, i) => (
                <span key={i} className="bg-gold-500 text-dark-900 text-sm px-3 py-1 rounded-full font-medium">
                  🎉 {f}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div>
            <p className="text-sm font-semibold text-gold-600 mb-2">{getT('modules.huangli.suitable')}</p>
            <div className="flex flex-wrap gap-2">
              {data.suitable.map((item, i) => (
                <span key={i} className="bg-gold-500/5 text-gold-600 text-sm px-3 py-1 rounded-full border border-gold-500/25">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div>
            <p className="text-sm font-semibold text-gold-600 mb-2">{getT('modules.huangli.avoid')}</p>
            <div className="flex flex-wrap gap-2">
              {data.avoid.map((item, i) => (
                <span key={i} className="bg-gold-500/5 text-gold-700 text-sm px-3 py-1 rounded-full border border-gold-500/25">
                  {item}
                </span>
              ))}
            </div>
      ...
      <div className="bg-white rounded-xl border border-gold-500/25 p-4 mb-4">
        <h2 className="text-base font-bold text-gold-600 mb-1">{getT('huangliPage.auspiciousHourTitle')}</h2>
        ...
        {(()=>{const sc=genShiChen(data.ganZhiDay.charAt(0),data.ganZhiDay.charAt(1),data.dayOfYear);return(
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {sc.map((s,i)=>(
              <div key={i} className={`rounded-lg px-3 py-2 text-xs border ${s.starGod.includes("吉")?"bg-gold-500/5 border-gold-500/60":"bg-gold-500/5 border-gold-500/50"} hover:shadow-sm transition-shadow`}>
                <div className="flex items-center gap-1.5 mb-0.5">
                  <span className="text-sm font-bold text-gray-800">{s.name}</span>
                  <span className="text-[10px] text-gray-400">{s.timeRange}</span>
                  <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded-full font-medium ${s.starGod.includes("吉")?"bg-gold-500/10 text-gold-600":"bg-gold-500/10 text-gold-700"}`}>{s.starGod.substring(0,2)}</span>
                </div>
                ...
                <div className="flex gap-2 mt-1 text-[10px]">
                  <span className="text-gold-600">{getT('huangliPage.suitable')} {s.suitable}</span>
                  <span className="text-gold-600">{getT('huangliPage.avoid')} {s.avoid}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4">
      <div className="bg-white rounded-xl border border-gold-500/25 p-4 mb-4">
        <h2 className="text-base font-bold text-gold-600 mb-1">{getT('huangliPage.solarTermsTitle')}</h2>
        ...
        <div className="divide-y divide-gray-100">
          {SOLAR_TERMS.map((st,i)=>{
            ...
            const sb=se==="春"?"bg-gold-500/5 border-gold-500/25 text-gold-600":se==="夏"?"bg-gold-500/5 border-gold-500/25 text-gold-700":se==="秋"?"bg-gold-500/5 border-gold-500/25 text-gold-600":"bg-gold-500/5 border-gold-500/25 text-gold-600"
            const bgs=sb.split(" ");const si=bgs[0];const bi=bgs[1];const ti=bgs[2]
            return(
              <div key={i} className={`flex items-start gap-3 py-2.5 px-1 rounded-lg ${se===getSeason(month)?"bg-gold-500/5 -mx-1 px-2":""}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {se===getSeason(month)&&<span className="text-[10px] font-medium text-gold-700 ml-auto">● {getT('huangliPage.currentLabel')}</span>}
                  </div>
