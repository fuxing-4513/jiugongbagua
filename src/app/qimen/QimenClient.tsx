...
export default function QimenClient() {
  ...
  return (<div className="max-w-2xl mx-auto px-4 py-10">
    ...
    <div className="bg-dark-800/80 backdrop-blur rounded-xl border border-dark-600 p-6 mb-8">
      ...
      <button onClick={calc} className="bg-gold-600 hover:bg-gold-500 text-dark-900 font-semibold px-6 py-2.5 rounded-lg transition-colors active:scale-95">排盘</button>
    </div>
    ...
    {r && (<div className="space-y-4">
      ...
      <div className="bg-dark-800/80 backdrop-blur rounded-xl border border-dark-600 p-4">
        ...
        <p className="text-[11px] text-gray-300 leading-relaxed">
          ...
          宜{QIMEN_DOORS[r.doorIndex].meaning.split('宜')[1]?.split('。')[0] || '平稳行事'}。
          坐向宜朝向{QIMEN_DOORS[r.doorIndex].dir}方或{QIMEN_STARS[r.starIndex].dir}方，
          避{QIMEN_DOORS.find(d=>d.luck==='大凶')?.dir || '对冲'}方。
        </p>
      </div>
    </div>)}
  </div>)
