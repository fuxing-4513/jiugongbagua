...
export default function QimenClient() {
  ...
  return (<div className="max-w-2xl mx-auto px-4 py-10">
    <h1 className="text-3xl font-bold text-gold-400 font-serif mb-3">{getT('qimenPage.title')}</h1>
    <p className="text-gray-400 mb-6">{getT('qimenPage.subtitle')}</p>

    <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-6 mb-8">
      <p className="text-xs text-gray-400 mb-3">{getT('qimenPage.selectHour')}</p>
      ...
      <button onClick={calc} className="bg-gold-600 hover:bg-gold-500 text-dark-900 font-semibold px-6 py-2.5 rounded-lg transition-colors active:scale-95">{getT('qimenPage.chart')}</button>
    </div>
    ...
    {r && (<div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4 text-center">
          <p className={`text-sm font-bold ${QIMEN_DOORS[r.doorIndex].luck==='大吉'?'text-gold-600':QIMEN_DOORS[r.doorIndex].luck==='凶'||QIMEN_DOORS[r.doorIndex].luck==='大凶'?'text-gold-600':'text-gold-500'}`}>{QIMEN_DOORS[r.doorIndex].name}门</p>
          <p className="text-[10px] text-gray-500">{QIMEN_DOORS[r.doorIndex].dir} · {QIMEN_DOORS[r.doorIndex].luck}</p>
        </div>
        <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4 text-center">
          <p className={`text-sm font-bold ${QIMEN_STARS[r.starIndex].luck==='大吉'?'text-gold-600':QIMEN_STARS[r.starIndex].luck==='凶'?'text-gold-600':'text-gold-500'}`}>{QIMEN_STARS[r.starIndex].name}星</p>
        ...
        <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4 text-center">
          ...
          <p className="text-[10px] text-gray-500">{getT('qimenPage.spirit')}</p>
        </div>
      </div>

      <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4">
        <h3 className="text-xs font-semibold text-gold-400 mb-2">{getT('qimenPage.eightDoorsMeaning')}</h3>
        <p className="text-xs text-gray-300">{QIMEN_DOORS[r.doorIndex].meaning}</p>
      </div>
      <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4">
        <h3 className="text-xs font-semibold text-gold-400 mb-2">{getT('qimenPage.nineStarsMeaning')}</h3>
        <p className="text-xs text-gray-300">{QIMEN_STARS[r.starIndex].meaning}</p>
      </div>

      <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-4">
        <h3 className="text-xs font-semibold text-gray-200 mb-2">{getT('qimenPage.comprehensiveAdvice')}</h3>
