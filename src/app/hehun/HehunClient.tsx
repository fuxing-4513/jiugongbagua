          ...
          <div key={i} className={'bg-dark-800/80 rounded-xl border p-5 '+(i===0?'border-gold-500/20':'border-gold-500/40')}>
            ...
            <div className="flex gap-1.5 mb-3">
              {['木','火','土','金','水'].map(wx => {
                ...
                return <div key={wx} className="flex-1"><div className="text-center text-[9px] mb-0.5"><span className={'font-semibold '+(WXC[wx]||'text-gray-400')}>{wx}</span></div><div className={'h-12 rounded overflow-hidden flex flex-col justify-end '+(WXBG[wx]||'bg-dark-700')}><div className={'rounded-t bg-gradient-to-t '+(WXBAR[wx]||'from-gold-500 to-gold-700')} style={{height:Math.max(pct,5)+'%'}}></div></div><div className="text-center text-[9px] text-gray-600">{cnt.toFixed(1)}</div></div>
              })}
            </div>
