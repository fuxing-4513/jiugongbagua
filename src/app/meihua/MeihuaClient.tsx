...
export default function MeihuaClient() {
  ...
  return (<div className="max-w-2xl mx-auto px-4 py-10">
    ...
    <div className="bg-dark-800/80 rounded-xl border border-dark-600 p-5 mb-8">
      ...
      {error && <p className="text-xs text-gold-600 mb-2">{error}</p>}
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-dark-700/40 rounded-lg p-3">
                  <p className="text-[10px] text-gold-600 font-medium mb-1">💼 {locale === 'en' ? 'Career' : locale === 'ja' ? '事業' : locale === 'ko' ? '사업' : '事业'}</p>
                  <p className="text-[11px] text-gray-300 leading-relaxed">{duan.career}</p>
                </div>
                <div className="bg-dark-700/40 rounded-lg p-3">
                  <p className="text-[10px] text-gold-600 font-medium mb-1">❤️ {locale === 'en' ? 'Love' : locale === 'ja' ? '感情' : locale === 'ko' ? '감정' : '感情'}</p>
                  <p className="text-[11px] text-gray-300 leading-relaxed">{duan.love}</p>
                </div>
                <div className="bg-dark-700/40 rounded-lg p-3">
                  <p className="text-[10px] text-gold-600 font-medium mb-1">🌿 {locale === 'en' ? 'Health' : locale === 'ja' ? '健康' : locale === 'ko' ? '건강' : '健康'}</p>
                  <p className="text-[11px] text-gray-300 leading-relaxed">{duan.health}</p>
                </div>
                <div className="bg-dark-700/40 rounded-lg p-3">
                  <p className="text-[10px] text-gold-600 font-medium mb-1">💰 {locale === 'en' ? 'Wealth' : locale === 'ja' ? '財運' : locale === 'ko' ? '재운' : '财运'}</p>
