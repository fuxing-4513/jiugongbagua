...
function ssEffect(shenSha: string[], key: string): ShenShaEffect {
  const has = shenSha.some(s => s.includes(key))
...
function comprehensiveAnalysis(dg: string, dz: string, wx: Record<string,number>, pillars: any[], zodiac: string, lunar: any, monthZhi: string, shenSha: string[], gender: string): BaziAnalysis {
  const dw = wxM[dg]
          ...
          <div className="space-y-1.5">
            {result.shenSha.map((s:string,i:number)=>(
              <span key={i} className={`inline-block text-xs mr-1.5 mb-1 px-2 py-1 rounded ${
                ...
                s.includes('孤辰')?'bg-gray-700 text-gray-400 border border-dark-600':'bg-dark-700 text-gray-400 border border-dark-600'
              }`}>{s}</span>
            ))}
          </div>
