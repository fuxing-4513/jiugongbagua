...
⚠️ **风险警示**
${dw === '火' || dw === '木' ? '不宜高杠杆投资，宜稳健理财为主。' : '宜分散投资，避免单一项目投入过多。'}【VIP可查看完整财库开合时机和流年财星分析】`;
    },
  },
  {
...
【VIP可解锁完整的十年大运详解，包含每一年十二宫星曜流转分析】`;
    },
  },
  {
...
【VIP可查看逐月流年详解、每月吉凶事项及趋吉避凶具体建议】`;
  ...
  },
  {
    ...
    generate:(data?: unknown) => {
      ...
      const dw = (WU_XING as Record<string,string>)[dz.gan];
      const greeting = d.name ? `${d.name}，` : '';
      return `${greeting}基于日主${dz.gan}（${dw}）的五行属性，提供风水布局建议：

🏠 **居家风水**
• 吉方位：${dw === '木' ? '东方（木旺）、北方（水生木）' : dw === '火' ? '南方（火旺）、东方（木生火）' : dw === '土' ? '中央（土旺）、南方（火生土）' : dw === '金' ? '西方（金旺）、中央（土生金）' : '北方（水旺）、西方（金生水）'}
• 卧室宜朝此方向，有助于睡眠和能量恢复

🎨 **色彩搭配**
• 宜用色：${dw === '木' ? '绿色、浅蓝、原木色' : dw === '火' ? '红色、橙色、紫色' : dw === '土' ? '黄色、棕色、米色' : dw === '金' ? '白色、银色、金色' : '蓝色、黑色、深灰'}
• 避用色：${dw === '木' ? '大面积白色、金色' : dw === '火' ? '大面积黑色、深蓝' : dw === '土' ? '大面积绿色' : dw === '金' ? '大面积红色、紫色' : '大面积黄色、棕色'}

🌿 **植物与摆件**
• 推荐植物：${dw === '木' ? '绿萝、发财树、富贵竹' : dw === '火' ? '红掌、玫瑰、火炬花' : dw === '土' ? '金琥仙人球、多肉植物' : dw === '金' ? '白掌、茉莉、百合' : '睡莲、龟背竹、吊兰'}
• 适宜摆件：${dw === '木' ? '木质工艺品、书卷、青龙摆件' : dw === '火' ? '红色饰品、蜡烛、朱雀摆件' : dw === '土' ? '陶瓷器皿、黄水晶、稳定重物' : dw === '金' ? '金属摆件、铜器、白虎摆件' : '鱼缸、流水摆件、玄武摆件'}

【VIP可获取完整的住宅九宫飞星布局图、每年流年吉凶方位及化解方法】`;
    },
  },
];
