...
🔐 **VIP 深度内容**：逐月流年详解 · 每月吉凶事项清单 · 太岁/岁破/三煞方位 · 趋吉避凶具体行动指南`;
  ...
  // ═══ FREE: 灵签 ═══
  {
    key:'lingqian', label:'观音灵签', icon:'🏮', free:true,
    generate:(data?: unknown) => {
      const d = extractData(data);
      const greeting = d.name ? `${d.name}，` : '';
      // 基于出生数据生成一个1-100的灵签号
      const seed = d.year * 10000 + d.month * 100 + d.day;
      const qianNum = ((seed * 7 + 13) % 100) + 1;
      const qianLabels: Record<string,string[]> = {
        '1':['上上','日出扶桑','万事如意，大吉大利'],
        '2':['中平','鲲化为鹏','待时而动，终成大器'],
        '3':['下下','众星拱月','坚守本心，静待光明'],
      };
      const idx = qianNum <= 35 ? '1' : qianNum <= 70 ? '2' : '3';
      const [level, title, desc] = qianLabels[idx];
      return `${greeting}您今日抽取${level}灵签第${qianNum}签——「${title}」

🪷 **签文**
${qianNum <= 20 ? '云开见月明，花发上林春。\n龙飞天汉表，鹏转九霄云。' : qianNum <= 40 ? '春雷起蛰龙，风送滕王阁。\n一举登科日，双亲未老时。' : qianNum <= 60 ? '月出照关山，秋风入汉关。\n行人归未得，鸿雁几时还。' : qianNum <= 80 ? '松柏耐岁寒，梅花雪里看。\n守得云开日，方知天地宽。' : '海上生明月，天涯共此时。\n清风来不远，细雨润无声。'}

📿 **解签**
${desc}。
${level === '上上' ? '此签大吉，凡谋必遂，所求皆得。事业上有贵人相助，财运亨通，感情顺遂。宜积极进取，把握良机。' : level === '中平' ? '此签中平，虽有波折终有转机。当前可能遇到一些阻力，但只要坚持不懈，守正待时，自会云开月明。' : '此签暗示当前时机未到，宜潜龙勿用。不是不报，时候未到。保持信心，厚积薄发，转机将在坚持中到来。'}

🎐 **运势指引**
• 事业：${level === '上上' ? '有升迁或突破之象，宜主动出击' : level === '中平' ? '稳中求进，注意人际关系' : '宜保守，等待更好的时机'}
• 财运：${level === '上上' ? '财运亨通，正财偏财均有收获' : level === '中平' ? '收支平衡，不宜大额投资' : '宜节流守成，避免冲动消费'}
• 感情：${level === '上上' ? '姻缘和美，单身者有望遇良缘' : level === '中平' ? '需多沟通理解，避免误会' : '宜顺其自然，强求反失'}
• 健康：${level === '上上' ? '身心安康，精力充沛' : '注意劳逸结合，适度运动'}

🕯️ **建议**
常怀善念，广结善缘。${level === '上上' ? '好运眷顾有心人。' : '风雨过后是彩虹。'}`;
    },
  },
];
