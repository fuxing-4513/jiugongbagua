...
const LIU_HE_REN_XING: Record<string, string[]> = {
  ...
  '卯戌': [
    ...
    '卯戌合入命，说明你从小被家里管得严，长大了对朋友也是掏心掏肺。',
  ],
  '辰酉': [
...
const SHI_SHEN_MONTH: Record<string, string> = {
  ...
  '比肩':'月柱比肩，你这人到哪儿都有朋友，不势利眼，实干派。',
  '劫财':'月柱劫财，你这人社交能力强，竞争心重，朋友有难你第一个上。',
}

const SHI_SHEN_HOUR: Record<string, string> = {
  '食神':'时柱食神，你这人喜欢自己琢磨东西，对朋友大方，就是有点完美主义。',
...
function describeBiJie(riGan: string, pills: PillarInfo[]): string[] {
  ...
  if (ssM[riGan]?.[monthGan] === '比肩' || ssM[riGan]?.[monthGan] === '劫财') {
    ...
    r.push(`月干${biType}占位，${biType === '比肩' ? '说明你这人对朋友真心实意，身边不缺兄弟伙，关键时候有人替你挡事' : '说明你朋友虽多但得长个心眼，别因朋友破财'}`)
  }
...
function describeGuiRen(riGan: string, pills: PillarInfo[]): string[] {
  ...
  // 年上贵人分析
  r.push(`年柱${yearP.gz}是你的贵人位，看上去有靠山，但离得远，真正使上劲要看缘分。`)
  ...
  for (const kz of kuZhi) {
    if (yearZhi === kz) {
      ...
      if (count >= 2) {
        r.push(`年上和家里共${kz}这个根，说明你祖上有阴德，关键时刻有贵人出手。`)
      } else {
        // 检查日主是否借根于年上
        r.push(`年上${kz}是${kz === '辰'?'水库':kz === '戌'?'火库':kz === '丑'?'金库':'木库'}，但没有跟家里连上根，得等大运流年到了才能真正借上力。`)
      }
    }
  }
...
export function analyzeRiZhuGenJi(pills: PillarInfo[], riGan: string): string[] {
  ...
  if (BEST_ROOT[bestRootKey]) {
    r.push(`你能借力的位置是${BEST_ROOT[bestRootKey]}`)
  }
  ...
  if (foundKu.length > 0) {
    for (const k of foundKu) {
      ...
      r.push(`${pos}柱${k}是你的${riWx}出处 — ${pos === '年' ? '说明你祖上有点底子，祖上能帮你' : pos === '月' ? '说明你父母和朋友圈能给你撑腰' : pos === '日' ? '说明你自己就是自己的靠山' : '说明你晚年不愁，儿女运不错'}`)
    }
  }
