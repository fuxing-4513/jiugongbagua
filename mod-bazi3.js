...
// Find doCalc function's setError('') call
const searchStr = "setError('')\n    const y = parseInt(year)";
const searchStr2 = "setError('')\r\n    const y = parseInt(year)";

let idx = c.indexOf(searchStr);
let hasCR = false;
if (idx === -1) {
  idx = c.indexOf(searchStr2);
  hasCR = true;
}

if (idx === -1) {
  console.log('Could not find doCalc setError');
  process.exit(1);
}

console.log('Found at:', idx);

const branchCode = `setError(''); setResult(null)
    if (mode === 'bazi') {
      try {
...
const replacement = branchCode + (hasCR ? "\r\n    const y = parseInt(year)" : "\n    const y = parseInt(year)");
c = c.substring(0, idx) + replacement + c.substring(idx + searchStr.length);

// Now do form replacement
const oldFormOpen = '<div className="flex gap-3 mb-4">';
const newFormOpen = '<div className="flex gap-2 mb-4 flex-wrap">';
// Replace the outer div opening
const formIdx = c.indexOf(oldFormOpen);
console.log('Form at:', formIdx);

if (formIdx > 0) {
  // Find the end of the form section (the button) - look for the next "开始算命" button
  const submitBtn = '开始算命</button>';
  const formEndIdx = c.indexOf(submitBtn, formIdx);
  console.log('button at:', formEndIdx);
  
  if (formEndIdx > 0) {
    const formSection = c.substring(formIdx, formEndIdx + submitBtn.length);
    console.log('Form section length:', formSection.length);
    console.log('First 80 chars:', JSON.stringify(formSection.substring(0, 80)));
    ...
    c = c.substring(0, formIdx) + newForm + c.substring(formEndIdx + submitBtn.length);
